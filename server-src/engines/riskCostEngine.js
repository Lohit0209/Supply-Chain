...
function calculateSegmentImpact(segment, disruptions = []) {
  ...
  // Recalculate true distance of this segment based on all waypoints (including detours)
  let trueDistanceKm = 0;
  for (let i = 0; i < points.length - 1; i++) {
    trueDistanceKm += distanceKm(points[i].lat, points[i].lng, points[i+1].lat, points[i+1].lng);
  }
  
  // Update segment stats based on actual path length
  const updatedSegment = { 
    ...segment, 
    distanceKm: Math.round(trueDistanceKm),
    durationHours: segment.mode === 'air' ? trueDistanceKm / 850 : (segment.mode === 'sea' ? trueDistanceKm / 35 : trueDistanceKm / 65)
  };

  if (!disruptions || disruptions.length === 0) return { ...updatedSegment, isImpacted: false };
  ...
  if (impactingZones.length > 0) {
    const costImpact = Math.round(updatedSegment.distanceKm * 0.8 * maxPenalty);
    const timeImpact = (updatedSegment.durationHours || 1) * 0.5 * maxPenalty;
  ...
  return { ...updatedSegment, isImpacted: false };
}
