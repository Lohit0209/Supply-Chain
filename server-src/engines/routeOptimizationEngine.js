...
function applyDetours(points, disruptions) {
  ...
  for (let i = 0; i < points.length - 1; i++) {
    ...
    const steps = Math.ceil(dist / 25); // Higher resolution (every 25km) for precise collision detection
...
function detourAroundZone(points, D) {
  ...
  // Increase clearance to be well outside the disruption (at least 2.2x radius)
  const clearanceKm = radiusKm * 2.2; 
  ...
  const startIdx = Math.max(0, firstAffected - 15); // Start curving ~375km before intersection for a very smooth approach
  const endIdx = Math.min(points.length - 1, lastAffected + 15);
  ...
  const peakFactor = clearanceKm * 1.35; // Wider arc to ensure safe clearance from threat influence
  ...
  // Pick the peak that is FURTHER from the disruption center to ensure we always curve OUTWARD
  const chosenPeak = dist1 > dist2 ? peak1 : peak2;

  const arc = [];
  const numArcPoints = 24; // Ultra-smooth arc for high-fidelity pathing
...
function buildItinerary(segments) {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (seg.mode !== 'road' && !seg.isSubSegment) {
...
function generateRoutes(input, disruptions = []) {
  ...
  const buildDisplayPathWithDisruptions = (segments) => {
    return segments.flatMap(seg => {
      let points = [];
      if (seg.mode === 'air') points = getFlightWaypoints(seg.origin, seg.dest, 25);
      ...
      // Apply arcing detours for sea/road segments to ensure ships "deroute" safely around high-threat zones
      if (seg.mode !== 'air' && disruptions && disruptions.length > 0) {
        points = applyDetours(points, disruptions);
      }
      
      // Optimization: Only scan for land detection if it's a SEA segment. 
      // User request: "every flight path should be purple regardless of it being over land or water"
      if (seg.mode !== 'sea') {
        const p = calculateSegmentImpact({
          ...seg,
          points: points
        }, disruptions);
        return [p];
      ...
      // We process SEA segments through the splitting logic to detect land transitions (Land Bridges)
      ...
      for (let i = 0; i < points.length - 1; i++) {
        ...
        // Define isLand check using bounding boxes from hubs.js
        const isPointInCanal = (CANAL_ZONES || []).some(cz => {
          const d = distanceKm(midLat, midLng, cz.lat, cz.lng);
          return d < (cz.radius || 100); 
        });

        const isPointOnLand = (LAND_REGIONS || []).some(reg => 
        ...
        // Only SEA modes can transition to ROAD visually to identify land crossings
        if (seg.mode === 'sea' && isPointOnLand && !isPointInCanal) {
          finalMode = 'road';
        }
      ...
      for (let i = 1; i < rawSubSegments.length; i++) {
        ...
        if (ss.mode === currentGroup.mode) {
          ...
          currentGroup = {
            ...
            isSubSegment: true, // Mark as sub-segment to avoid itinerary bloat
  ...
  const r1Itin = buildItinerary(r1Display); // Use high-res points for itinerary accuracy
  const route1 = {
    id: 'fastest',
    ...
    totalDistanceKm: r1Display.reduce((s, seg) => s + (seg.distanceKm || 0), 0),
    totalTimeDays: r1Itin.totalDays,
    cost: calculateRouteCost(r1Display, weight, itemType, destRegion),
    totalCarbonKg: calculateRouteCarbon(r1Display, weight),
  ...
  const r2Itin = buildItinerary(r2Display);
  const route2 = {
    id: 'cheapest',
    ...
    totalDistanceKm: r2Display.reduce((s, seg) => s + (seg.distanceKm || 0), 0),
    totalTimeDays: r2Itin.totalDays,
    cost: calculateRouteCost(r2Display, weight, itemType, destRegion),
    totalCarbonKg: calculateRouteCarbon(r2Display, weight),
    risk: calculatePathRisk(r2Display.flatMap(d => d.points), itemType, fragility, disruptions),
    modes: [...new Set(r2Display.map(s => s.mode))],
  ...
  const r3Itin = buildItinerary(r3Display);
  const route3 = {
    id: 'lowest_risk',
    ...
    totalDistanceKm: r3Display.reduce((s, seg) => s + (seg.distanceKm || 0), 0),
    totalTimeDays: r3Itin.totalDays,
    cost: calculateRouteCost(r3Display, weight, itemType, destRegion),
    totalCarbonKg: calculateRouteCarbon(r3Display, weight),
  ...
  const r4Itin = buildItinerary(r4Display);
  const route4 = {
    id: 'balanced',
    ...
    totalDistanceKm: r4Display.reduce((s, seg) => s + (seg.distanceKm || 0), 0),
    totalTimeDays: r4Itin.totalDays,
    cost: calculateRouteCost(r4Display, weight, itemType, destRegion),
    totalCarbonKg: calculateRouteCarbon(r4Display, weight),
    risk: calculatePathRisk(r4Display.flatMap(d => d.points), itemType, fragility, disruptions),
    modes: [...new Set(r4Display.map(s => s.mode))],
