...
export const TacticalCommandCenter: React.FC<Props> = () => {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Fetch current supply chain context
      const contextRes = await fetch('/api/gemini/context');
      if (!contextRes.ok) throw new Error('Failed to retrieve tactical context');
      const { shipments, disruptions } = await contextRes.json();

      // 2. Prepare the AI Prompt
      const systemPrompt = `You are CodAIx — an enterprise-grade Supply Chain Tactical Intelligence system. 
Analyze real-time shipment data and disruptive events to suggest precise rerouting decisions.

ACTIVE SHIPMENTS (${shipments.length} total):
${JSON.stringify(shipments, null, 2)}

SYSTEM DISRUPTIONS:
${disruptions.length ? JSON.stringify(disruptions, null, 2) : 'None registered yet.'}

USER INCIDENT REPORT: "${msg}"
...
Respond in EXACT JSON format:
{
  ...
  "unaffectedShipments": ["string[] ids"],
  "suggestedSystemDisruption": {
    "name": "string", "lat": number, "lng": number, "radius": number,
    "riskType": "geopolitical | weather | contact | piracy | operational", "riskScore": number
  }
}`;

      // 3. Call Gemini Model directly from Frontend
      ...
      // 5. Notify server of the new disruption if detected
      if (analysis.suggestedSystemDisruption) {
        ...
        }).catch(() => {}); // Silent fail for server registration
      }

    } catch (err: any) {
      console.error('[Gemini Front-end Error]', err);
      let errorMsg = err.message;
      if (err.message?.includes('403') || err.message?.includes('PERMISSION_DENIED')) {
        errorMsg = "API Key Error: Forbidden (403). Please ensure your Gemini API Key is correctly configured in the Secrets panel and that 'Generative AI API' is enabled.";
      } else if (err.message?.includes('429')) {
        errorMsg = "Quota Exceeded: Too many requests. Please wait a minute.";
      }
      
      setMessages(prev => [...prev, {
        role: 'assistant',
        text: `Tactical Analysis Interrupted: ${errorMsg}`,
        timestamp: Date.now(),
        isError: true,
      }]);
    } finally {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 420, background: 'var(--bg-surface)', borderRadius: 14, overflow: 'hidden', border: '1px solid var(--border-dim)' }}>
      ...
      <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--border-dim)', display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(0,0,0,0.2)' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 9, color: 'var(--text-muted)', marginTop: 1 }}>Powered by Gemini 1.5 · Real-time shipment intelligence</div>
        </div>
      </div>
