...
const MapController = ({ center, zoom, bounds, onMapClick }) => {
  ...
  useEffect(() => {
    // We use a serialized key to check if the target view has actually changed.
    // This prevents the map from snapping back to the same location every few seconds.
    const targetKey = bounds ? JSON.stringify(bounds) : `${center?.[0]},${center?.[1]},${zoom}`;
    if (lastTargetRef.current === targetKey) return;
    
    if (bounds && bounds.length > 0) {
      map.fitBounds(bounds, { padding: [100, 100], animate: true, duration: 2.0 });
...
const MapVisualization = ({ 
  ...
  useEffect(() => {
    // 1. Always zoom if a specific route is selected for preview
    ...
    // 2. Only zoom to shipments ONCE when they first appear, or if the fleet list changes
    else if (shipments.length > 0 && !hasAutoZoomedRef.current) {
    ...
    // Reset auto-zoom if fleet becomes empty, so next fleet gets zoomed
  ...
  return (
    <div style={{ height: '100%', width: '100%', position: 'relative', borderRadius: 'inherit', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', top: '16px', right: '16px', zIndex: 1000, display: 'flex', flexDirection: 'column', gap: '8px' }}>
        <button 
          onClick={() => {
            const b = selectedRoute?.displaySegments.flatMap(s => s.points).map(p => [p.lat, p.lng]) || 
                      shipments.flatMap(s => s.route.displaySegments.flatMap(ds => ds.points)).map(p => [p.lat, p.lng]);
            if (b && b.length > 0) setBounds(b);
          }}
