...
router.get('/context', (req, res) => {
  try {
    ...
    res.json({
      ...
      disruptions: activeDisruptions.map(d => ({ name: d.name, lat: d.lat, lng: d.lng, type: d.riskType, severity: d.riskScore }))
    });
  } catch (err) {
...
router.post('/analyze', async (req, res) => {
  ...
  // Check multiple possible secret names just in case
  const key = (process.env.GEMINI_API_KEY || process.env.MY_GEMINI_KEY || '').trim();
...
Respond in this EXACT JSON format (no markdown code fences, pure JSON):
{
  ...
  "suggestedSystemDisruption": {
    ...
    let rawText = '';
    if (!key || key.length < 5) {
      throw new Error('API_KEY_MISSING: No valid Gemini API Key found in settings.');
    }
    ...
    try {
      ...
      const isKeyError = msg.includes('403') || msg.toLowerCase().includes('key') || msg.includes('API_KEY_INVALID');
      
      console.warn('[Gemini API Error]', msg);
      
      let summary = "The Tactical Intelligence suite has encountered a technical interruption.";
      if (isQuota) {
        summary = "API Quota Exceeded — The tactical engine is operating in fallback mode due to high volume. Wait a moment and try again.";
      } else if (isKeyError) {
        summary = "Security Validation Failed — The API key provided was rejected (Forbidden 403). Please ensure you have a fresh key pasted correctly in Settings.";
      } else {
        summary = `Intelligence Stream Interrupted: ${msg}. Operating via contingency logic.`;
      }
      ...
      const mockResult = {
        ...
        disruptionClassification: "Operational Interruption",
